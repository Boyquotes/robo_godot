...
# REFERÊNCIAS ÀS COORDENADAS NA INTERFACE DO GODOT
...
var pose_changed = false # se a posição do servidor mudou em relação à posição da simulação
var extra_coordinates = {'r': 0, 'j1': 0, 'j2': 0, 'j3': 0, 'j4': 0} # guarda coordenadas não utilizadas pela simulação
...
func _process(delta):
	...
	# Atualiza valores das coordenadas na interface
	...
	# Se a posição mudar, move o robô para a nova posição
	if pose_changed:
		move_robot()
		pose_changed = false
...
# Move robô interpolando as posições intermediárias
func move_robot():
	playing = true
	var poses = [last_pose]
	for i in poses.size():
...
# Roda quando a requisição termina
func _on_HTTPRequest_request_completed(result, response_code, headers, body):
	var json = JSON.parse(body.get_string_from_utf8()) # Pega json
	
	# Salva coordenadas extras para a interface
	...
	# Altera valores de posição na variável utilizada para movimento, se a posição for diferente
...
# Faz requisição toda vez que o timer acaba (de 1 em 1 segundo)
